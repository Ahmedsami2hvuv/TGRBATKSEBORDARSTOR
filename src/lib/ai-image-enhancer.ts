...
/**
 * فحص وتقييم صورة الباب عبر Gemini Flash Latest API المستقر 100%
 */
...
قم بتحليل الصورة المرفقة وأجب بصيغة JSON فقط:
{
  ...
  // المسارات الرسمية المعتمدة لـ latest في جوجل API
  const endpoints = [
    ...
    for (const ep of endpoints) {
      try {
        ...
        if (response.ok) {
          ...
          const modelName = ep.split("/models/")[1]?.split(":")[0] || "gemini-1.5-flash";
