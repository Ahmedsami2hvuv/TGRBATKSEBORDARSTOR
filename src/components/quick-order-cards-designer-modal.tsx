...
export function QuickOrderCardsDesignerModal({
  ...
  // إذا كان التبويب النشط هو "العنصر المخصص" وانتقل المستخدم لعنصر غير مخصص، نرجعه تلقائياً لتبويب "تحريك"
  useEffect(() => {
    if (activeTool === "custom" && !selectedElementKey.startsWith("custom_")) {
      setActiveTool("move");
    }
  }, [selectedElementKey, activeTool]);
    ...
    return (
      <div
        onClick={(e) => {
          ...
          if (!elemKey.startsWith("custom_") && activeTool === "custom") {
            setActiveTool("move");
          }
        }}
        ...
        <div className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-950/90 border border-amber-500/30 rounded-2xl p-3 sm:p-4 shadow-xl">
                <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-800">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="hidden sm:inline text-[11px] text-slate-400 font-bold">
                ...
                <div className="relative max-w-2xl mx-auto p-1 sm:p-2 bg-slate-900/40 rounded-2xl border border-slate-800">
                  ...
                  {activeCard === "shop_card" && (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 items-start relative z-10">
                        <div className="flex flex-col gap-2">
                          <InteractiveElement elemKey="headerShopInfo" title="كبسولة عنوان المحل" cfg={config?.shopCard?.headerShopInfo} className="w-fit">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={config?.shopCard?.headerShopInfo?.imageUrl || "/images/order-luxury/shop-card/header-shop-info.webp"} alt="المحل" className="h-8 sm:h-9 w-auto object-contain pointer-events-none" />
                          </InteractiveElement>
                          ...
                          <InteractiveElement elemKey="btnShopLocation" title="زر موقع المحل" cfg={config?.shopCard?.btnShopLocation} className="w-fit">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={config?.shopCard?.btnShopLocation?.imageUrl || "/images/order-luxury/shop-card/btn-shop-location.webp"} alt="موقع المحل" className="h-7 sm:h-8 w-auto object-contain pointer-events-none" />
                          </InteractiveElement>
                          <div className="grid grid-cols-2 gap-1.5 pt-1">
                            <InteractiveElement elemKey="btnCall" title="زر الاتصال" cfg={config?.shopCard?.btnCall}>
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img src={config?.shopCard?.btnCall?.imageUrl || "/images/order-luxury/shop-card/btn-call.webp"} alt="اتصال" className="h-7 sm:h-8 w-auto object-contain pointer-events-none" />
                            </InteractiveElement>
                            <InteractiveElement elemKey="btnWhatsapp" title="زر الواتساب" cfg={config?.shopCard?.btnWhatsapp}>
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img src={config?.shopCard?.btnWhatsapp?.imageUrl || "/images/order-luxury/shop-card/btn-whatsapp.webp"} alt="واتساب" className="h-7 sm:h-8 w-auto object-contain pointer-events-none" />
                            </InteractiveElement>
                          </div>
                        </div>
                        <div className="flex flex-col items-center gap-2">
                          <InteractiveElement elemKey="photoContainer" title="إطار صورة باب المحل" cfg={config?.shopCard?.photoContainer} className="w-full">
                          ...
                          <div className="grid grid-cols-2 gap-1.5 w-full">
                            <InteractiveElement elemKey="btnCamera" title="زر كاميرا الباب" cfg={config?.shopCard?.btnCamera}>
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img src={config?.shopCard?.btnCamera?.imageUrl || "/images/order-luxury/shop-card/btn-camera.webp"} alt="كاميرا" className="h-7 w-auto object-contain pointer-events-none mx-auto" />
                            </InteractiveElement>
                            <InteractiveElement elemKey="btnGallery" title="زر معرض الباب" cfg={config?.shopCard?.btnGallery}>
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img src={config?.shopCard?.btnGallery?.imageUrl || "/images/order-luxury/shop-card/btn-gallery.webp"} alt="معرض" className="h-7 w-auto object-contain pointer-events-none mx-auto" />
                            </InteractiveElement>
                          </div>
                        </div>
                      </div>
              ...
              {/* ================= 2. لوحة التحكم السفلية المدمجة (Docked Bottom Control Panel) =================
                  مثبّتة أسفل منطقة المعاينة أثناء التمرير (sticky) بحيث تبقى دائماً في متناول الإصبع
                  على الموبايل دون الحاجة للنزول أسفل الشاشة، وتُقسّم الأدوات إلى تبويبات مدمجة صغيرة
                  (تحريك / حجم / دوران ولون / العنصر المخصص) بدلاً من عرضها كلها دفعة واحدة. */}
              <div className="sticky bottom-0 z-40 -mx-2 sm:-mx-4 mt-3 bg-gradient-to-t from-slate-950 via-slate-950/98 to-slate-950/95 backdrop-blur-md border-t-2 border-amber-500/40 rounded-t-3xl shadow-[0_-10px_35px_rgba(0,0,0,0.6)] px-2.5 sm:px-4 pt-2.5 pb-[max(0.75rem,env(safe-area-inset-bottom))]">

                {/* صف علوي: اختيار العنصر + إخفاء/إظهار + ضبط افتراضي */}
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <select
                    ...
                    className="flex-1 min-w-0 bg-slate-900 border border-amber-500/40 text-amber-300 font-black text-[11px] sm:text-xs rounded-xl px-2 py-2 focus:ring-2 focus:ring-amber-400"
                  >
                    {elementsList.map((i) => (
                  ...
                  <button
                    ...
                    className={`shrink-0 h-9 w-9 rounded-xl text-sm transition flex items-center justify-center ${
                  ...
                  <button
                    ...
                    className="shrink-0 h-9 w-9 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm transition flex items-center justify-center"
                ...
                {/* محتوى التبويب النشط (منطقة قابلة للتمرير كإجراء احتياطي على الشاشات الصغيرة جداً) */}
                <div className="max-h-[38vh] overflow-y-auto pb-1">
                  ...
                  {activeTool === "move" && (
                    <div className="bg-slate-950 p-2.5 rounded-2xl border border-slate-800">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[11px] font-black text-amber-300">الموضع (X / Y)</span>
                      ...
                      <div className="flex items-center justify-center gap-4">
                        <div className="flex flex-col items-center gap-1.5">
                          <button
                            ...
                            className="w-12 h-9 bg-slate-800 hover:bg-amber-500 hover:text-black text-white font-black rounded-xl active:scale-90 transition flex items-center justify-center"
                            title="تحريك لأعلى"
                          >
                            ▲
                          </button>
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              className="w-9 h-9 bg-slate-900 border border-amber-500/40 text-amber-300 text-xs font-black rounded-xl active:scale-90 transition flex items-center justify-center"
                  ...
                  {activeTool === "size" && (
                    <div className="bg-slate-950 p-2.5 rounded-2xl border border-slate-800 space-y-2.5">
                      <div>
                        ...
                        <div className="flex items-center gap-1.5">
                          <button type="button" onClick={() => adjustScaleX(-0.05)} className="w-7 h-7 bg-slate-800 hover:bg-slate-700 text-white font-black rounded text-xs">-</button>
                          <input
                            type="range"
                            min="0.2"
                            max="2.5"
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5">
                          <button type="button" onClick={() => adjustScaleY(-0.05)} className="w-7 h-7 bg-slate-800 hover:bg-slate-700 text-white font-black rounded text-xs">-</button>
                          <input
                            type="range"
                            min="0.2"
                            max="2.5"
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5">
                          <button type="button" onClick={() => adjustScale(-0.05)} className="w-7 h-7 bg-slate-800 hover:bg-slate-700 text-white font-black rounded text-xs">-</button>
                          <input
                            type="range"
                            min="0.3"
                            max="2.5"
                  ...
                  {activeTool === "rotate" && (
                    <div className="bg-slate-950 p-2.5 rounded-2xl border border-slate-800 space-y-2.5">
                      <div>
                        ...
                        <div className="flex items-center gap-1.5">
                          <button type="button" onClick={() => adjustRotate(-5)} className="px-1.5 py-1 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded text-[10px]">-5°</button>
                          <input
                            type="range"
                            min="-180"
                            max="180"
                        ...
                        <div className="flex items-center justify-between gap-1 mt-1.5">
                          <button type="button" onClick={() => updateSelectedElementField("rotate", 0)} className="flex-1 px-2 py-1 bg-slate-900 border border-slate-700 text-[10px] font-bold text-slate-300 hover:text-white rounded">0° استقامة</button>
                          <button type="button" onClick={() => updateSelectedElementField("rotate", 90)} className="flex-1 px-2 py-1 bg-slate-900 border border-slate-700 text-[10px] font-bold text-slate-300 hover:text-white rounded">90° عمودي</button>
                          <button type="button" onClick={() => updateSelectedElementField("rotate", 180)} className="flex-1 px-2 py-1 bg-slate-900 border border-slate-700 text-[10px] font-bold text-slate-300 hover:text-white rounded">180° مقلوب</button>
                        </div>
                      </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1.5">
                            <input
                              ...
                              className="w-20 bg-slate-900 border border-slate-700 text-[10px] font-mono text-center rounded px-1 py-1 text-white"
                            />
                            <input
                              type="color"
                              value={selectedStyle.color || "#ffffff"}
                              onChange={(e) => updateSelectedElementField("color", e.target.value)}
                              className="h-7 w-9 bg-transparent rounded cursor-pointer border border-slate-700"
                              title="تغيير اللون"
                            />
                          </div>
                        </div>
                  ...
                  {activeTool === "custom" && isSelectedCustom && selectedCustomElement && (
                    <div className="bg-slate-950 p-2.5 rounded-2xl border border-amber-500/30 space-y-3">
                      <span className="text-[11px] font-black text-amber-300">✨ إعدادات العنصر المخصص والإجراء التفاعلي</span>
