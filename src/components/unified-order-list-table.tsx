...
function MoneyMiniBadges({ row }: { row: MandoubRow }) {
  ...
  return (
    <div className="absolute -top-2.5 -right-2 flex flex-wrap items-center gap-1" dir="rtl">
      ...
      {showAdminPickup ? (
        <span
          className={`${pillBase} bg-blue-600 text-white ring-blue-700`}
          title="صادر الإدارة"
        >
              ...
              if (currentDateStr !== lastDateStr) {
                ...
                separator = (
                  <tr key={`date-sep-${currentDateStr}`} className="bg-red-700 dark:bg-red-900 text-white">
                    <td colSpan={adjustedColCount} className="px-4 py-2.5 text-right text-sm font-black uppercase tracking-widest border-y-2 border-red-800 dark:border-red-950">
                  ...
                  >
                    ...
                    <td className="px-2 py-2.5">
                      <div className="max-w-[14rem] break-words sm:max-w-[18rem]">
                        ...
                        {/* أزرار اللوكيشن وصورة الباب انتقلت إلى عمود المنطقة تحت اسم المنطقة */}
                        {
                                ...
                                <CenterModal title="فتح الموقع الجغرافي لـ:" onClose={() => setActiveLocId(null)}>
                                  <div className="flex flex-col gap-1">
                                    {!isDoubleRoute && o.shopLocationUrl && o.shopLocationUrl.trim().length > 2 && (
                                      <a href={o.shopLocationUrl.trim()} target="_blank" className="flex min-h-[52px] items-center gap-3 px-4 py-3.5 text-sm sm:text-base font-black text-slate-800 hover:bg-rose-50 active:scale-[0.98] transition-all rounded-xl border border-slate-100 text-right w-full kse-location-btn-class">
                                        <span className="size-10 flex items-center justify-center rounded-full bg-sky-100 text-sky-600 text-xl shrink-0">🏢</span> موقع العميل (المحل)
                                      </a>
                                    )}
                                    {o.customerLocationUrl && o.customerLocationUrl.trim().length > 2 && (
                                      <a href={o.customerLocationUrl.trim()} target="_blank" className="flex min-h-[52px] items-center gap-3 px-4 py-3.5 text-sm sm:text-base font-black text-slate-800 hover:bg-rose-50 active:scale-[0.98] transition-all rounded-xl border border-slate-100 text-right w-full kse-location-btn-class">
                                        <span className="size-10 flex items-center justify-center rounded-full bg-emerald-100 text-emerald-600 text-xl shrink-0">👤</span> موقع الزبون
                                      </a>
                                    )}
                                    {o.secondCustomerLocationUrl && o.secondCustomerLocationUrl.trim().length > 2 && (
                                      <a href={o.secondCustomerLocationUrl.trim()} target="_blank" className="flex min-h-[52px] items-center gap-3 px-4 py-3.5 text-sm sm:text-base font-black text-slate-800 hover:bg-rose-50 active:scale-[0.98] transition-all rounded-xl border border-slate-100 text-right w-full kse-location-btn-class">
                                        <span className="size-10 flex items-center justify-center rounded-full bg-violet-100 text-violet-600 text-xl shrink-0">👥</span> موقع الزبون 2 / مستلم
                                      </a>
                                    )}
