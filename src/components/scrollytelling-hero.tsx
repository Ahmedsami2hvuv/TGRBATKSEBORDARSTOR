...
export default function ScrollytellingHero() {
  ...
  const h1Opacity = useTransform(p, [0, 0.08], [1, 0]);
  const h1Y = useTransform(p, [0, 0.08], [0, -20]);
  
  const h2Opacity = useTransform(p, [0.65, 0.7, 0.8, 0.85], [0, 1, 1, 0]);
  const h2Y = useTransform(p, [0.65, 0.7, 0.8, 0.85], [20, 0, 0, -20]);

  // Final Scene
  const finalOpacity = useTransform(p, [0.9, 0.95], [0, 1]);
  const finalY = useTransform(p, [0.9, 0.95], [20, 0]);
  ...
  return (
    <div ref={containerRef} className="h-[280vh] relative w-full font-['IBM_Plex_Sans_Arabic']" style={{ background: "#F6FAFD", color: "#22323F" }}>
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex flex-col items-center justify-center">
        ...
        {/* Headlines */}
        <motion.div style={{ opacity: h1Opacity, y: h1Y }} className="absolute left-1/2 top-[16%] -translate-x-1/2 w-[90vw] md:w-[620px] text-center z-10 pointer-events-none">
        ...
        {/* Final Scene */}
        <motion.div style={{ opacity: finalOpacity, y: finalY }} className="absolute bottom-[8%] left-1/2 -translate-x-1/2 flex flex-col items-center gap-[12px] z-10 text-center w-[90vw] md:w-full">
          ...
          <p className="text-[#22323F]/80 text-[15px] md:text-[18px] max-w-[600px] font-medium leading-loose">
            وين ما كنت تفتح واتساب تراسلني وتطلب (أي شي) ويوصلك للباب!
          </p>
        </motion.div>
