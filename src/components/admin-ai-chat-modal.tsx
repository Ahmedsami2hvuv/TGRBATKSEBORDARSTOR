...
export function AdminAiChatModal({ isOpen, onClose, initialPosition }: AdminAiChatModalProps) {
  ...
  const [isMuted, setIsMuted] = useState(false); // مفعل الصوت افتراضياً
  ...
  const [messages, setMessages] = useState<ChatLogMessage[]>([
    {
      id: "init_msg",
      sender: "ai",
      text: "أهلاً بك يا أبو الأكبر! المساعد الصوتي والذكي جاهز لتنفيذ أوامرك فوراً بالصوت أو الكتابة! 🚀",
  ...
  // تعيين موضع النافذة الأولي عند الفتح
  useEffect(() => {
    if (isOpen) {
      if (initialPosition) {
        const winWidth = typeof window !== "undefined" ? window.innerWidth : 500;
        const winHeight = typeof window !== "undefined" ? window.innerHeight : 800;
        const targetX = Math.max(10, Math.min(winWidth - 395, initialPosition.x > winWidth / 2 ? initialPosition.x - 360 : initialPosition.x + 20));
        const targetY = Math.max(10, Math.min(winHeight - 560, Math.max(20, initialPosition.y - 120)));
  ...
  const handleMoveDrag = (e: MouseEvent | TouchEvent) => {
    ...
    const maxX = typeof window !== "undefined" ? window.innerWidth - 320 : 300;
      ...
      <div className="p-3 bg-slate-900 border-t border-slate-800 flex flex-col gap-2.5">
        ...
        <div className="flex items-center justify-between gap-2 px-1 py-1">
          ...
          <div
            ...
            title="انقر للتحدث أو اسحب للتحريك"
          >
          ...
          <button
            ...
            title="فتح / إغلاق الكتابة النصية"
          >
