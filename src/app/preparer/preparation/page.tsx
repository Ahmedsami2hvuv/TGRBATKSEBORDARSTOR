...
export default async function PreparerPreparationPage({ searchParams }: Props) {
  ...
  if (!v.ok) return (
    <div className="p-8 text-center">
      <p className="text-lg font-bold text-rose-700">الرابط غير صالح.</p>
      <p className="mt-2 text-sm text-slate-500">يرجى فتح الرابط الأصلي المرسل إليك.</p>
    </div>
  );
  ...
  // التحقق من صحة التوكن مقابل قاعدة البيانات
  if (!preparer || preparer.portalToken !== v.token) return (
    <div className="p-8 text-center">
      <p className="text-lg font-bold text-slate-800">الحساب غير متاح أو الرمز غير صحيح.</p>
      <p className="mt-2 text-sm text-slate-500">اطلب رابطاً جديداً من المسؤول.</p>
    </div>
  );
  ...
  // جلب كل المحلات التي يملك المجهز صلاحية عليها (ليس فقط canSubmitOrders)
  const allShopIds = preparer.shopLinks.map((l) => l.shopId);
  // المحلات التي يمكنه تقديم طلبات فيها
  const submitShopIds = preparer.shopLinks.filter(l => l.canSubmitOrders).map((l) => l.shopId);
  const shopIds = submitShopIds.length > 0 ? submitShopIds : allShopIds;
  ...
  const [couriers, orderTable, webStorePending, drafts] = await Promise.all([
    ...
    loadPreparerPortalOrderTableData({
      preparerId: preparer.id, shopIds: allShopIds, orderListResetAt: preparer.orderListResetAt,
      tab: "all", wardFilter: "lower", saderFilter: "lower", prepFilter: null, onlySubmittedByThisPreparer: true,
    }),
    shopIds.length > 0 ? prisma.order.findMany({
      where: {
        shopId: { in: shopIds },
        status: "pending",
  ...
  return (
    <div className="kse-app-inner mx-auto max-w-6xl px-3 py-4 pb-24 sm:px-4">
            ...
            {safeDrafts.map((d) => (
              <div key={d.id} className="flex gap-2 items-stretch">
                <FullscreenWalletLauncher
                  ...
                  title={d.titleLine || "مسودة"}
                >
                  <div className="min-w-0 flex-1 text-right">
                    <p className="text-sm font-black text-slate-900 dark:text-slate-100">{d.titleLine || "مسودة بدون عنوان"}</p>
                    <p className="mt-1 text-[11px] text-slate-500">{d.customerRegion?.name || ""} — {new Date(d.createdAt).toLocaleDateString("ar-IQ")}</p>
                  </div>
                  <span className={`rounded-lg px-2 py-1 text-[10px] font-black ${d.status === "priced" ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
                    {d.status === "priced" ? "مُسعَّرة ✅" : "مسودة 📝"}
                  </span>
                </FullscreenWalletLauncher>
                <form action={archivePreparerShoppingDraftAction} onSubmit={(e) => { if(!confirm("هل تريد حذف هذه المسودة؟")) e.preventDefault(); }}>
                  <input type="hidden" name="p" value={auth.p} /><input type="hidden" name="exp" value={auth.exp} /><input type="hidden" name="s" value={auth.s} />
                  <input type="hidden" name="draftId" value={d.id} />
                  <button type="submit" className="h-full rounded-xl border border-rose-200 bg-rose-50 px-3 text-xs font-black text-rose-700 hover:bg-rose-100 transition-colors">حذف</button>
                </form>
              </div>
            ))}
          </div>
      ...
      <section className="kse-glass-dark overflow-hidden border border-sky-200 shadow-sm dark:border-slate-800 rounded-2xl">
        <PreparerOrdersSection
          allRows={safeOrderTableRows}
