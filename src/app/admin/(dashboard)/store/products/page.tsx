  ...
  try {
      ...
      <div className="p-6 space-y-6" dir="rtl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            {branchId && (
              <Link href="/admin/store/products" className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-bold transition-colors">
                عرض الكل
              </Link>
            )}
            <Link href="/admin/store" className="px-4 py-2 bg-violet-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-violet-100 transition-all hover:scale-105 active:scale-95">
              + إضافة منتج
            </Link>
          </div>
        </div>
    ...
    return (
      <div className="p-20 text-center bg-white rounded-[2rem] border-2 border-dashed border-rose-100 m-6">
        ...
        <Link href="/admin/store/products" className="mt-6 inline-block px-6 py-2 bg-slate-900 text-white rounded-xl font-bold">تحديث الصفحة</Link>
      </div>
    );
  }
}
