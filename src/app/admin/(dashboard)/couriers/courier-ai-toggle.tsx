...
import { toggleCourierAI } from "./actions";
import { useRouter } from "next/navigation";
import { DynamicIcon } from "@/components/dynamic-icon";
import { GlobalIconsConfig } from "@/lib/icon-settings";

export function CourierAIToggle({
  courierId,
  initialDisabled,
  icons
}: {
  ...
  async function handleToggle() {
    ...
    setLoading(true);
    try {
      const res = await toggleCourierAI(courierId, !disabled);
      if (res.success) {
  ...
  return (
    <button
      ...
      disabled={loading}
      title={disabled ? "تفعيل الذكاء الاصطناعي لهذا المندوب" : "تعطيل الذكاء الاصطناعي لهذا المندوب"}
