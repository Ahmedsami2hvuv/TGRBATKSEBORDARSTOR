...
export async function GET() {
  try {
    console.log("Cleaning old store items via Prisma Server...");

    // 1. مسح كافة البيانات السابقة في المتجر
    ...
    // 2. إدخال الهيكلية والمنتجات الجديدة المطلوبة بالكامل
    ...
    return NextResponse.json({ ok: true, message: "تمت إضافة كافة المنتجات والأقسام بنجاح" });
  } catch (error: any) {
    console.error("Seed Store Error:", error);
    return NextResponse.json({ error: error.message || "حدث خطأ أثناء رفع بيانات المتجر" }, { status: 500 });
  }
}
