...
function buildInfoHref(phone: string, regionId?: string | null) {
  ...
  if (regionId === null || regionId === undefined) {
    return `${SECRET_ADMIN_PATH}/customers/info?phone=${p}`;
  }
  const r =
    regionId === NO_REGION_SLUG
      ? NO_REGION_SLUG
      : encodeURIComponent(regionId);
  return `${SECRET_ADMIN_PATH}/customers/info?phone=${p}&regionId=${r}`;
}
  ...
  const tableRows =
    regionIdParam !== "" && ordersFiltered
      ? ordersFiltered.map((o) => {
          const missingLoc = !hasCustomerLocationUrl(o.customerLocationUrl, o.customer?.customerLocationUrl);
          return {
            id: o.id,
      ...
      {regionIdParam === "" ? (
        <>
          ...
          <div className={`${ad.section} space-y-2`}>
            ...
            <p className="text-sm">
              <Link
                href={`${SECRET_ADMIN_PATH}/customers/orders?phone=${encodeURIComponent(phone)}`}
                className={ad.link}
              >
        ...
        <>
          ...
          {regionIdParam !== NO_REGION_SLUG && profileForRegion ? (
            <div className={`${ad.section} space-y-4`}>
              ...
              <p className="text-sm">
                <Link
                  href={`${SECRET_ADMIN_PATH}/customers/profiles/${profileForRegion.id}/edit`}
                  className={ad.link}
                >
