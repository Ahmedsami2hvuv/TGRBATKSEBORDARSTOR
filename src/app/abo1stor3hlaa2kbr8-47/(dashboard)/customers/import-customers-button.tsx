...
export function ImportCustomersButton({ icons }: { icons: GlobalIconsConfig | null }) {
  ...
  async function handleReset() {
    ...
    setStatus("resetting");
    try {
      await fetch(`/api${SECRET_ADMIN_PATH}/import/customers/reset`, { method: "POST" });
      router.refresh();
  ...
  async function handleCheck() {
    ...
    try {
      ...
      const res = await fetch(`/api${SECRET_ADMIN_PATH}/import/customers/check`, { signal });
  ...
  async function handleImport() {
    ...
    try {
      ...
      while (true) {
        ...
        const res = await fetch(`/api${SECRET_ADMIN_PATH}/import/customers/import-missing`, {
          method: "POST",
          signal,
  ...
  async function handleSyncPhotos() {
    ...
    try {
      const signal = startCancelableTask();
      const checkRes = await fetch(`/api${SECRET_ADMIN_PATH}/import/customers/sync-photos/check`, { signal });
  ...
  async function handleRunSyncPhotos() {
    ...
    try {
      ...
      while (currentOffset < total) {
        if (cancelRequestedRef.current) break;
        const res = await fetch(`/api${SECRET_ADMIN_PATH}/import/customers/sync-photos`, {
          method: "POST",
          signal,
          headers: { "Content-Type": "application/json" },
  ...
  async function handleCleanUrls() {
    setStatus("checking");
    try {
      const res = await fetch(`/api${SECRET_ADMIN_PATH}/import/customers/clean-urls`, { method: "POST" });
      const data = await res.json();
      if (data.success) {
        alert(
          `تم التنظيف بنجاح!\n` +
