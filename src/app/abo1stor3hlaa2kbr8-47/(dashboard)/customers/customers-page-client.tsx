...
export function CustomersPageClient({
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    router.push(`${SECRET_ADMIN_PATH}/customers?${params.toString()}`);
  };
  ...
  const getPageLink = (p: number) => {
    ...
    return `${SECRET_ADMIN_PATH}/customers?${params.toString()}`;
  };
                  ...
                  <tbody>
                    {profiles.map((p) => (
                      <tr key={p.id} className="border-b border-slate-100 text-slate-800 align-top">
                        ...
                        <td className="py-2">
                          <Link href={`${SECRET_ADMIN_PATH}/customers/profiles/${p.id}/edit`} className={`${ad.link} flex items-center gap-1`}>
