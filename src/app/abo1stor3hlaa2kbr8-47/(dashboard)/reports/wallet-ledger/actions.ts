...
function revalidateWalletReportsAfterMutation(orderIdForPaths?: string) {
  revalidatePath(`${SECRET_ADMIN_PATH}/reports/wallet-ledger`);
...
function safeReturnUrl(u: string): string {
  const t = u.trim();
  if (t.startsWith(`${SECRET_ADMIN_PATH}/reports/wallet-ledger`)) return t;
  return `${SECRET_ADMIN_PATH}/reports/wallet-ledger`;
}
...
export async function purgeWalletLedgerData(
  ...
  revalidatePath(`${SECRET_ADMIN_PATH}/reports/wallet-ledger`);
  revalidatePath(`${SECRET_ADMIN_PATH}/reports/general`);
  revalidatePath(`${SECRET_ADMIN_PATH}/reports/courier-mandoub`);
  revalidatePath("/mandoub", "layout");
  revalidatePath("/preparer", "layout");
  return { ok: true };
}
