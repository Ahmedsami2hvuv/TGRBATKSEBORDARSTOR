...
export function ImportShopsButton() {
  ...
  async function generateMissingReport() {
    ...
    try {
      const res = await fetch("/api/abo1stor3hlaa2kbr8-47/import/shops/report", { signal: startCancelableTask() });
  ...
  async function importMissingShops() {
    ...
    try {
      ...
      while (!isDone) {
        ...
        const res = await fetch("/api/abo1stor3hlaa2kbr8-47/import/shops", {
          method: "POST",
          signal,
  ...
  async function syncPhotosToR2() {
    ...
    try {
      ...
      while (!isDone) {
        ...
        const res = await fetch("/api/abo1stor3hlaa2kbr8-47/import/shops/sync-photos", {
          method: "POST",
          signal,
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ offset, limit })
        ...
        <button
          onClick={() => {
            if(confirm("سيتم مسح كافة المحلات، هل أنت متأكد؟")) {
              fetch("/api/abo1stor3hlaa2kbr8-47/import/reset", { method: "POST" }).then(() => window.location.reload());
            }
          }}
