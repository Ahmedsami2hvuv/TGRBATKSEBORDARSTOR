...
export function ImportShopsButton() {
  ...
  async function handleCheck() {
    setStatus("loading");
    try {
      const res = await fetch("/api/abo1stor3hlaa2kbr8-47/import/shops/check");
  ...
  async function handleImport() {
    setStatus("loading");
    try {
      const res = await fetch("/api/abo1stor3hlaa2kbr8-47/import/shops", { method: "POST" });
      const data = await res.json();
      if (data.success) {
        alert(`تم سحب ${data.count} محل بنجاح! جاري سحب الزبائن الآن تلقائياً...`);
        // بعد سحب المحلات، نسحب الزبائن فوراً
        const resCust = await fetch("/api/abo1stor3hlaa2kbr8-47/import/customers", { method: "POST" });
  ...
  async function handleReset() {
    ...
    setStatus("loading");
    try {
      const res = await fetch("/api/abo1stor3hlaa2kbr8-47/import/reset", { method: "POST" });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <div className="flex gap-2 items-center">
      ...
      {status === "confirming" ? (
        <div className="flex gap-2 items-center bg-green-50 p-1 rounded-lg border border-green-200">
          <span className="text-sm font-bold px-2 text-green-700">وجدنا {foundCount} محل. سحب شامل؟</span>
          <button onClick={handleImport} className="bg-green-600 text-white px-3 py-1 rounded-md font-bold">نعم، ابدأ</button>
