            ...
            <li key={emp.id} className="py-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-col w-full sm:hidden gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    {emp.whatsappLink && emp.whatsappLink.trim().length > 10 ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        title="إرسال الرابط للواتساب"
                        className="inline-flex items-center justify-center rounded-lg bg-emerald-600 p-2 text-white hover:bg-emerald-700 transition-colors"
                      >
                        <DynamicIcon iconKey="ui_whatsapp" config={icons} fallback="💬" className="w-4 h-4" />
                      </a>
                    ) : (
                      <span className="text-[10px] text-rose-500 bg-rose-50 px-2 py-1 rounded" title="رقم غير صالح">
                        ⚠️
                      </span>
                    )}
                    {emp.orderPortalUrl && emp.orderPortalUrl.trim().length > 5 && (
                      <a
                        ...
                        className="inline-flex items-center justify-center rounded-lg bg-sky-100 p-2 text-sky-700 hover:bg-sky-200 transition-colors"
                      >
                        <DynamicIcon iconKey="ui_link" config={icons} fallback="🔗" className="w-4 h-4" />
                      </a>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between w-full">
                  ...
                  {/* أزرار بجانب الرقم في وضع الهاتف */}
                  <div className="flex items-center gap-1.5">
                    {/* نسخ الرابط */}
                    {emp.orderPortalUrl && emp.orderPortalUrl.trim().length > 5 && (
                      <button
                        ...
                        title="نسخ الرابط"
                        className="inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white p-2 text-slate-600 hover:bg-slate-50 transition-colors"
                      >
                        {copiedId === emp.id ? (
                          <DynamicIcon iconKey="ui_success" config={icons} fallback="✅" className="w-4 h-4" />
                        ) : (
                          <DynamicIcon iconKey="ui_copy" config={icons} fallback="📋" className="w-4 h-4" />
                        )}
                      </button>
                    )}
                    {/* تعديل */}
                    <Link
                      ...
                      title="تعديل العميل"
                      className="inline-flex items-center justify-center rounded-lg bg-slate-800 p-2 text-white hover:bg-slate-900"
                    >
                    ...
                    <form action={renewEmployeeOrderPortalToken}>
                      ...
                      >
                        <DynamicIcon iconKey="ui_refresh" config={icons} fallback="🔄" className="w-4 h-4" />
                      </button>
                    </form>
                    {/* حذف */}
                    <form action={deleteEmployee} onSubmit={(ev) => !confirm(`حذف الموظف "${emp.name}" نهائياً؟`) && ev.preventDefault()}>
                      ...
                      <button
                        type="submit"
                        title="حذف العميل"
                        className="inline-flex items-center justify-center rounded-lg bg-rose-50 p-2 text-rose-600 hover:bg-rose-100"
                      >
              ...
              {/* --- وضع الكمبيوتر/التابلت (شاشات متوسطة وأكبر) --- */}
