...
export function ImportRegionsButton({ icons }: { icons: GlobalIconsConfig | null }) {
  ...
  async function handleCheck() {
    ...
    try {
      ...
      const res = await fetch(`/api${SECRET_ADMIN_PATH}/import/regions/check?t=${Date.now()}`, { signal: createSignal() });
  ...
  async function handleImport() {
    setStatus("importing");
    try {
      const res = await fetch(`/api${SECRET_ADMIN_PATH}/import/regions`, { method: "POST", signal: createSignal() });
      const data = await res.json();
      if (data.success) {
