    ...
    <div className="space-y-4 rounded-2xl border-2 border-amber-300 bg-amber-50/90 p-5 shadow-xl text-right" dir="rtl">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-amber-200 pb-4">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => setShowBulkAdd(!showBulkAdd)} className="rounded-xl bg-violet-600 text-white px-3 py-1.5 text-[10px] font-black shadow-sm transition active:scale-95 hover:bg-violet-700">➕ قائمة كاملة</button>
      ...
      <div className="grid gap-2 bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
        <div className="grid sm:grid-cols-[1fr_auto] gap-3 items-end">
          <label className="flex flex-col gap-1">
            <span className="text-[10px] font-black text-slate-600">اختر المجهز للتعيين</span>
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <button type="button" onClick={assignSelectedProductsToPreparer} disabled={!productAssigneeId || selectedProductIndexes.length === 0} className="rounded-xl bg-sky-600 text-white px-4 py-2 text-xs font-black shadow-sm transition disabled:opacity-50 hover:bg-sky-700">
            ...
            <div key={i} className={isSelected ? "ring-2 ring-sky-400 rounded-2xl" : ""}>
              ...
              {isEditing && !deleteMode && (
                <div className="mt-2 bg-white p-4 rounded-xl border-2 border-amber-400 grid grid-cols-2 gap-3 shadow-inner animate-in slide-in-from-top-2">
                  ...
                  <button type="button" onClick={() => setEditingIndex(null)} className="col-span-2 bg-slate-800 text-white rounded-lg py-2 text-xs font-black active:scale-95 transition-transform">حفظ السطر</button>
                </div>
              )}
            </div>
          );
...
export function PendingOrdersClient({
  ...
  return (
    <div className="space-y-6">
      {orders.map((order) => (
        <div key={order.id} className="kse-glass-dark rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-3">
            <div className="min-w-0 flex-1">
              <h3 className="text-lg font-black text-slate-900">طلب #{order.orderNumber}</h3>
              <p className="text-sm font-bold text-slate-700 mt-1">{order.shopName}</p>
              <p className="text-xs text-slate-500 mt-1">{order.regionName}</p>
            </div>
            <div className="flex gap-2">
              <RejectButton orderId={order.id} />
              {order.preparerShoppingJson && (
                <button type="button" onClick={() => window.location.href = `${SECRET_ADMIN_PATH}/orders/${order.id}/edit`} className="rounded-xl bg-amber-500 px-3 py-1.5 text-xs font-bold text-white shadow-sm transition hover:bg-amber-600">
                  <span>💰</span>
