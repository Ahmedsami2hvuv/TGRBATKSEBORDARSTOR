      ...
      <section className={ad.section}>
        <form className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4" method="get">
          ...
          <div className="flex items-end gap-2">
            ...
            <Link href={`${SECRET_ADMIN_PATH}/search`} className={ad.btnDark}>
              تصفير
            </Link>
          </div>
        </form>
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                {result.shops.map((s) => (
                  <Link
                    key={s.id}
                    href={`${SECRET_ADMIN_PATH}/shops/${s.id}/edit`}
                    className="block rounded-xl border border-sky-200 bg-white p-3 hover:bg-sky-50"
                  >
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                {result.regions.map((r) => (
                  <Link
                    key={r.id}
                    href={`${SECRET_ADMIN_PATH}/regions/${r.id}/edit`}
                    className="block rounded-xl border border-sky-200 bg-white p-3 hover:bg-sky-50"
                  >
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                {result.couriers.map((c) => (
                  <Link
                    key={c.id}
                    href={`${SECRET_ADMIN_PATH}/couriers`}
                    className="block rounded-xl border border-sky-200 bg-white p-3 hover:bg-sky-50"
                  >
                    <p className="font-bold text-slate-900">
                      {c.name} — {c.phone}
