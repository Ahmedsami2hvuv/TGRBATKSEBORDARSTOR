...
export default async function BranchPage(props: { params: Promise<{ id: string }> }) {
  ...
  let branchRaw: any = null;
  let settingsRaw: any = null;

  try {
    settingsRaw = await prisma.globalSettings.findUnique({ where: { id: "system" } }).catch(() => null);
    branchRaw = await prisma.storeBranch.findUnique({
      where: { id: branchId },
      include: {
        category: true,
        products: {
    ...
    console.error("[BranchPage DB Error]:", error);
  }

  // البحث في البيانات الاحتياطية في حال تعذر الداتابيز أو عدم وجود الفرع بها
  if (!branchRaw) {
    for (const cat of FALLBACK_STORE_DATA) {
      const fb = cat.branches.find(b => b.id === branchId || b.name.includes(branchId));
      if (fb) {
        branchRaw = {
          id: fb.id,
          name: fb.name,
          ...
          profitMargin: 0,
          category: { id: cat.id, name: cat.name, profitMargin: 0 },
          subBranches: [],
  ...
  if (!branchRaw) {
    return (
      <div className="text-center py-20" dir="rtl">
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">الفرع غير موجود أو غير نشط</h2>
