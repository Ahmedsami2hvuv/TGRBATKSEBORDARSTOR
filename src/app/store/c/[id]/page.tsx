...
export default async function CategoryPage(props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (categoryRaw) {
      ...
      category = safeJson(categoryRaw);
      branches = safeJson(branchesRaw);
      slides = safeJson(slidesRaw);
    }
  } catch (error) {
    console.error("[CategoryPage Fetch Error]:", error);
  }

  // إذا لم نجد في الداتابيز، نقوم بالبحث في البيانات المحدثة الثابتة
  if (!category) {
    const fallbackCat = FALLBACK_STORE_DATA.find(c => c.id === categoryId || c.name.includes(categoryId));
    if (fallbackCat) {
      category = { id: fallbackCat.id, name: fallbackCat.name, photoUrl: fallbackCat.photoUrl };
      branches = fallbackCat.branches.map(b => ({
        id: b.id,
        name: b.name,
        photoUrl: b.photoUrl,
        _count: { products: b.products.length }
      }));
    }
  }
