...
object LocationHelper {
    ...
    fun fetchAndSendLocation(context: Context) {
        ...
        val savedUrl = sharedPreferences.getString("mandob_url", null) ?: return
        val savedId = sharedPreferences.getString("mandob_id", null) ?: return

        try {
            val uri = Uri.parse(savedUrl)
            val c = uri.getQueryParameter("c") ?: savedId
            ...
            if (bestLocation != null && (System.currentTimeMillis() - bestLocation.time) < 60000) {
                ...
                sendLocationToServer(baseUrl, c, exp, s, bestLocation.latitude, bestLocation.longitude)
                return
            }
            ...
            val locationListener = object : LocationListener {
                ...
                override fun onLocationChanged(location: Location) {
                    if (!isSent) {
                        isSent = true
                        sendLocationToServer(baseUrl, c, exp, s, location.latitude, location.longitude)
                        try {
    ...
    private fun sendLocationToServer(baseUrl: String, c: String, exp: String, s: String, lat: Double, lng: Double) {
        val url = "$baseUrl/api/courier/location"
        val json = JSONObject()
        json.put("c", c)
